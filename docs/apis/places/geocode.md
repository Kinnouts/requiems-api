...
convert coordinates back into addresses (reverse geocoding).
Powered by [Nominatim](https://nominatim.openstreetmap.org/) (OpenStreetMap).
...
The `city` field is resolved from the Nominatim `address` block in priority order:
`city` → `town` → `village` → `county`. This handles rural areas where no formal
city is present.
...
- Address geocoding works best when the query includes country context (e.g. "Eiffel Tower, Paris, France" rather than just "Eiffel Tower").
- Reverse geocoding precision depends on OSM coverage density at the given coordinates. Cities and major towns are well covered worldwide.
- Nominatim ToS requires a `User-Agent` header and caching — both are handled automatically by the API.
