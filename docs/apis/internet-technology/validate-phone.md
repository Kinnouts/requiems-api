...
When the number is invalid, `valid` is `false` and the optional fields are
omitted:
...
- **Data Normalization** - Store phone numbers in consistent international
  format
...
calling code prefixed with a plus sign (E.164 format), for example +12015551234
or +447400123456. Numbers without a country code cannot be validated reliably.
...
**What happens when the number is invalid?** The API still returns HTTP 200 with
valid set to false. The country, type, and formatted fields are omitted from the
response.
