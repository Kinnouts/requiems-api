...
Returns historical and current annual average prices for 16 major commodities —
precious metals, energy, and agricultural goods. Data is sourced from
...
- **Prices are annual averages**, not real-time spot prices. Daily and monthly
  FRED observations are averaged per calendar year.
- **`change_24h`** is the year-over-year percentage change between the latest
  annual average and the prior year's average, despite the field name.
- **Up to 10 years** of historical data are returned alongside the current
  value.
- **Precious metals** (gold, silver, platinum, palladium) are priced per troy
  ounce.
- **Copper and coffee** are converted from FRED's USD/metric ton and USD/kg to
  USD/lb.
...
Re-running is safe (idempotent upsert). Run annually to refresh prices with the
latest FRED and Yahoo Finance data.
