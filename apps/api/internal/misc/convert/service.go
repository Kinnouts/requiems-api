...
// Convert converts value from one unit to another and returns a ConvertResponse.
func (s *Service) Convert(from, to string, value float64) (ConvertResponse, error) {
	fromKey := strings.ToLower(strings.TrimSpace(from))
	toKey := strings.ToLower(strings.TrimSpace(to))

	fromInfo, ok := units[fromKey]
	if !ok {
		return ConvertResponse{}, fmt.Errorf("unknown unit: %s", from)
	}

	toInfo, ok := units[toKey]
	if !ok {
		return ConvertResponse{}, fmt.Errorf("unknown unit: %s", to)
	}

	if fromInfo.category != toInfo.category {
		return ConvertResponse{}, fmt.Errorf("cannot convert %s to %s: incompatible unit types", from, to)
	}
	...
	if fromInfo.category == categoryTemperature {
		...
		result = roundTo(value*factor, 10)
...
func convertTemperature(from, to string, value float64) (float64, string) {
	...
	case "celsius→fahrenheit":
		return roundTo(value*9.0/5.0+32, 10), "°C × 9/5 + 32"
	case "fahrenheit→celsius":
		return roundTo((value-32)*5.0/9.0, 10), "(°F − 32) × 5/9"
	case "celsius→kelvin":
		return roundTo(value+273.15, 10), "°C + 273.15"
	case "kelvin→celsius":
		return roundTo(value-273.15, 10), "°K − 273.15"
	case "fahrenheit→kelvin":
		return roundTo((value-32)*5.0/9.0+273.15, 10), "(°F − 32) × 5/9 + 273.15"
	case "kelvin→fahrenheit":
		return roundTo(value*9.0/5.0-459.67, 10), "°K × 9/5 − 459.67"
	}
...
// roundTo rounds v to at most decimalPlaces decimal places.
